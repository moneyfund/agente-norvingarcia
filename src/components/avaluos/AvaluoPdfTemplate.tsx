...
const resolveAvaluoDate = (value: any) => {
...
const date = (v: any) => resolveAvaluoDate(v).toLocaleDateString('es-NI', { year: 'numeric', month: 'long', day: 'numeric' });
...
function TerrainInfoSection({ title, icon: Icon = FileText, items, wide = false }: any) { const visible = items.filter(([_, v]: any) => clean(v)); if (!visible.length) return null; return <div className={`terrain-info-section ${wide ? 'terrain-info-section--wide' : ''}`}><h3><Icon />{title}</h3><div className='terrain-info-rows'>{visible.map(([k, v]: any) => <div key={k} className='terrain-info-row'><span>{k}</span><strong>{text(v)}</strong></div>)}</div></div>; }
function CoefGroup({ title, items, icon: Icon = BarChart3, partLabel = '' }: any) { if (!items.length) return null; const sub = subtotalCoef(items); return <div className='terrain-coef-group'><h3><Icon />{title}{partLabel ? ` — ${partLabel}` : ''}<b>Subtotal {impact(sub)}</b></h3>{items.map((coef: any, i: number) => <div className='terrain-coef-row' key={`${title}-${i}`}><span>{labelize(coef.factor || coef.nombre)}</span><em>{coef.valorAplicado || '—'}</em><strong>{coef.impacto || impact(coef.coeficiente)}</strong></div>)}</div>; }
...
const splitLargeCoefGroups = (groups: any[], maxRows = 14) => groups.flatMap((group) => {
  if (!group?.items?.length || group.items.length <= maxRows) return [group];
  const chunks = [];
  for (let i = 0; i < group.items.length; i += maxRows) {
    chunks.push({ ...group, items: group.items.slice(i, i + maxRows), partLabel: `Parte ${chunks.length + 1}` });
  }
  return chunks;
});
...
export default function AvaluoPdfTemplate({ avaluo }: { avaluo: any }) {
  ...
  if (isTerreno) {
    ...
    const rawDate = avaluo?.createdAt || avaluo?.fecha || avaluo?.updatedAt || avaluo?.fechaEmision || avaluo?.createdAtServer || new Date();
    const fechaEmision = date(rawDate);
    ...
    const technicalPages = paginateBlocks(technicalSections, 64);
    const coefPageGroups = paginateBlocks(splitLargeCoefGroups(visibleCoefGroups).map((g) => ({ ...g, weight: groupWeight(g.items, 3) })), 34);
    const pages = [
      { content: <><div className='terrain-cover-top'><img src='/logo.png' className='terrain-logo' /><div><p>DIAMANTES REALTY GROUP</p><h1>INFORME TÉCNICO DE AVALÚO DE TERRENO</h1><h2>Informe profesional de valoración inmobiliaria</h2></div></div><div className='terrain-cover-photo'><Img src={avaluo?.imagenPrincipalBase64} /></div><div className='terrain-cover-bottom'><div><h3>{avaluo?.titulo || 'Terreno evaluado'}</h3><p>DIAMANTES REALTY GROUP desarrolla análisis inmobiliarios con enfoque técnico, lectura comercial y presentación corporativa para orientar decisiones patrimoniales, venta, compra y negociación de activos.</p></div><div className='terrain-cover-facts'><MiniFact label='Ciudad' value={avaluo?.ciudad}/><MiniFact label='Barrio / Zona' value={barrio}/><MiniFact label='Fecha' value={fechaEmision}/><MiniFact label='Agente' value={nombreAgente}/><MiniFact label='Teléfono' value={telefonoAgente}/></div></div></>, title: undefined },
      { title: 'Resumen ejecutivo', content: <><div className='terrain-summary-hero'><span>Valor estimado</span><strong>{money(avaluo?.valorFinal)}</strong><p>Resultado técnico preliminar para orientar estrategia comercial, publicación y negociación.</p></div><div className='terrain-value-grid'><ValueTile label='Valor mínimo sugerido' value={money(avaluo?.rangoMercado?.minimo || avaluo?.lowValue)}/><ValueTile label='Valor recomendado de negociación' value={money(avaluo?.precioObjetivoCierre)}/><ValueTile label='Valor recomendado de publicación' value={money(avaluo?.precioRecomendadoPublicacion)}/><ValueTile label='Nivel de confianza' value={avaluo?.nivelConfianza}/><ValueTile label='Clasificación' value={avaluo?.zonaSnapshot?.clasificacion || avaluo?.clasificacionZona}/><ValueTile label='Liquidez' value={avaluo?.indiceLiquidez ? `${avaluo.indiceLiquidez}/100` : c.liquidez}/><ValueTile label='Tiempo estimado de venta' value={avaluo?.tiempoEstimadoVenta}/><ValueTile label='Potencial de crecimiento' value={avaluo?.potencialCrecimiento}/><ValueTile label='Uso recomendado' value={c.usoPotencial}/></div></> },
      ...technicalPages.map((sections: any[], index: number) => ({ title: index === 0 ? 'Ficha técnica del terreno' : 'Ficha técnica del terreno (continuación)', content: <div className='terrain-info-grid'>{sections.map((section: any) => <TerrainInfoSection key={section.title} title={section.title} icon={section.icon} items={section.items} wide={section.title === 'Legal'} />)}</div> })),
      { title: 'Análisis profesional', content: <><div className='terrain-analysis'>{analysis.map((p: string, i: number) => <p key={i}>{p}</p>)}</div><TerrainCharts avaluo={avaluo} groups={visibleCoefGroups}/></> },
      ...coefPageGroups.map((groupSet: any[], index: number) => ({ title: index === 0 ? 'Análisis de coeficientes' : 'Análisis de coeficientes técnicos', content: <><div className='terrain-coef-grid'>{groupSet.map((g: any) => <CoefGroup key={`${g.title}-${g.partLabel || ''}`} {...g} />)}</div>{index === coefPageGroups.length - 1 && <div className='terrain-total-box'><span>TOTAL</span><strong>Factor acumulado {impact(factorTotal)}</strong><p>Precio base: {money(avaluo?.valorBase || (Number(avaluo?.areaM2Convertida || 0) * Number(avaluo?.basePriceM2 || 0)))} · Precio final: {money(avaluo?.valorFinal)}</p></div>}</> })),
      { title: 'Valores comerciales', content: <><div className='terrain-value-grid terrain-value-grid--large'><ValueTile dark label='Precio total' value={money(valorFinalTerreno)}/><ValueTile label='Precio por m²' value={money(valorM2Terreno)}/>{usedManzanas && <ValueTile label='Precio por manzana' value={money(pricePerManzana)}/>}<ValueTile label='Valor mínimo' value={money(avaluo?.rangoMercado?.minimo || avaluo?.lowValue)}/><ValueTile label='Valor medio' value={money(avaluo?.valorFinal || avaluo?.estimatedValue)}/><ValueTile label='Valor máximo' value={money(avaluo?.rangoMercado?.maximo || avaluo?.highValue)}/><ValueTile label='Precio recomendado de publicación' value={money(avaluo?.precioRecomendadoPublicacion)}/><ValueTile label='Precio recomendado de cierre' value={money(avaluo?.precioObjetivoCierre)}/><ValueTile label='Precio mínimo negociable' value={money(avaluo?.precioMinimoNegociacion)}/></div><p className='terrain-note'>Cuando el avalúo se registra en manzanas se presentan precio por manzana, precio por m² y precio total. Cuando se registra en metros cuadrados se omite el precio por manzana para mantener consistencia comercial.</p></> },
      { title: 'Metodología y cierre', content: <><p className='terrain-method'>El modelo combina precio base territorial, normalización por tamaño, coeficientes de ubicación, condiciones físicas, accesibilidad, servicios, mercado, entorno ambiental y situación legal. La matriz completa de coeficientes permanece visible para trazabilidad técnica y control de consistencia.</p><div className='terrain-info-grid terrain-info-grid--two'><div className='terrain-info-section'><h3><ShieldCheck />Factores favorables</h3><p>{favorables.length ? favorables.join(', ') : 'No se identifican impulsos superiores al mercado base.'}</p></div><div className='terrain-info-section'><h3><Waves />Factores limitantes</h3><p>{limitantes.length ? limitantes.join(', ') : 'No se identifican castigos técnicos relevantes.'}</p></div></div><div className='avaluo-pdf-closing terrain-closing'><img src='/logo.png' className='logo-pdf logo-pdf--closing' /><h2>DIAMANTES REALTY GROUP</h2><p>Informe técnico preliminar elaborado para apoyar decisiones comerciales y patrimoniales. No sustituye avalúo certificado por perito autorizado cuando sea requerido por ley o institución financiera.</p><div className='avaluo-pdf-signature'>{nombreAgente && <strong>{nombreAgente}</strong>}<span>Agente evaluador</span>{telefonoAgente && <span>Tel. {telefonoAgente}</span>}<span>{fechaEmision}</span></div></div></> },
    ];
    const totalPages = pages.length;
    return <div className='avaluo-pdf-template terrain-pdf-template'>{pages.map((page, index) => <Page key={`terrain-page-${index}`} premium pageNumber={index + 1} totalPages={totalPages} title={page.title}>{page.content}</Page>)}</div>;
  }
