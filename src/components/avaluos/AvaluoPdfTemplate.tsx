...
function Img({ src, className = '', wrapperClassName = '' }: { src?: string, className?: string, wrapperClassName?: string }) {
  ...
  const content = !src || failed
    ...
    : <img crossOrigin='anonymous' src={src} className={`avaluo-pdf-img ${className}`} onError={() => setFailed(true)} />;
...
function Footer() { return <div className='avaluo-pdf-footer'><span>Norvin García · Real Estate</span><span>Informe técnico preliminar</span></div>; }
...
export default function AvaluoPdfTemplate({ avaluo }: { avaluo: any }) {
  ...
  return <div className='avaluo-pdf-template'>
    ...
    <Page title='Análisis profesional'><div className='avaluo-pdf-analysis'>{analisis}</div></Page>
    ...
    <Page title='Coeficientes y metodología'><div className='avaluo-pdf-stack'>{coefs.map((c: any, i: number) => <div key={i} className='avaluo-pdf-coef'><span>{c.factor || c.nombre}</span><span className='avaluo-pdf-coef-impact'>{c.impacto || impact(c.coeficiente)}</span></div>)}</div><p className='avaluo-pdf-paragraph avaluo-pdf-mt-24'>La metodología utiliza un modelo multicriterio que pondera ubicación, área, entorno, acceso, servicios, características físicas y coeficientes de mercado para construir una referencia técnica preliminar.</p><p className='avaluo-pdf-note'>Este informe corresponde a una estimación técnica preliminar generada mediante un modelo multicriterio de ponderación inmobiliaria. No sustituye un avalúo certificado emitido por perito valuador autorizado cuando la ley o una institución financiera lo requiera.</p></Page>
    <Page title='Cierre profesional'><div className='avaluo-pdf-center' style={{ marginTop: 80 }}><img src='/logo.png' className='logo-pdf' style={{ margin: '0 auto' }} /><p style={{ marginTop: 40, marginBottom: 0, color: '#0b1f3a', fontSize: 24, fontWeight: 800 }}>{agente}</p>{avaluo?.telefonoAgente && <p className='avaluo-pdf-location' style={{ justifyContent: 'center', color: '#475569' }}><Phone size={18} />{avaluo.telefonoAgente}</p>}<p className='avaluo-pdf-paragraph'>Fecha de emisión: {date(new Date())}</p><div className='avaluo-pdf-signature'>Firma del agente evaluador</div><p className='avaluo-pdf-paragraph' style={{ maxWidth: 560, margin: '62px auto 0' }}>Gracias por confiar en Norvin García Real Estate. Este documento se entrega como soporte técnico y comercial para la toma de decisiones inmobiliarias.</p></div></Page>
  </div>;
}
