...
export default function AvaluoPdfTemplate({ avaluo }: { avaluo: any }) {
  ...
  return <div className='avaluo-pdf-template'>
    ...
    <Page title='Datos generales'><div className='avaluo-pdf-executive'><FileText/><div><h3>Resumen ejecutivo</h3><p>El presente informe resume los principales factores territoriales, físicos y comerciales considerados para estimar el valor preliminar de la propiedad evaluada.</p></div></div><div className='avaluo-pdf-grid-3 avaluo-pdf-tight-grid'><Card icon={Home} label='Tipo' value={avaluo?.tipoPropiedad}/><Card icon={MapPin} label='Ciudad' value={avaluo?.ciudad}/><Card icon={MapPin} label='Zona' value={avaluo?.zona}/><Card icon={Building2} label='Clasificación urbana' value={avaluo?.zonaSnapshot?.clasificacion}/><Card icon={ShieldCheck} label='Tipo de entorno' value={c.entorno || c.tipoEntorno}/><Card icon={Ruler} label='Área' value={num(avaluo?.areaM2Convertida || c.areaTerreno || c.areaM2Convertida) + ' m²'}/><Card label='Unidad de área' value={avaluo?.unidadArea || c.unidadArea}/><Card icon={BadgeDollarSign} label='Valor por m²' value={money(avaluo?.valorM2)}/><Card icon={Award} label='Nivel de confianza' value={avaluo?.nivelConfianza}/></div>{!!galeria.length && <div className='avaluo-pdf-mt-24'><h3 className='avaluo-pdf-section-subtitle'>Galería de referencia</h3><div className='avaluo-pdf-grid-3'>{galeria.map((src: string, i: number) => <Img key={src || i} src={src} alt={`Imagen adicional ${i + 1} de la propiedad`} wrapperClassName='avaluo-pdf-gallery-img' />)}</div></div>}</Page>
