...
export const useAvaluoSubmission = (usuarioId?: string) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      const datosGuardar = { titulo: snapshot.data.titulo || `Avalúo de ${snapshot.tipoPropiedad}`, tipoPropiedad: snapshot.tipoPropiedad, agenteEvaluador: snapshot.data.agenteEvaluador, telefonoAgente: snapshot.data.telefonoAgente || '', ciudad: snapshot.data.ciudad || 'Matagalpa', zona: snapshot.data.zona, unidadArea: result.unidadArea ?? snapshot.data.unidadArea, areaOriginal: result.areaOriginal ?? snapshot.data.areaOriginal, areaM2Convertida: result.areaM2Convertida ?? snapshot.data.areaM2Convertida, createdAt: new Date().toISOString(), usuarioId, imagenPrincipalUrl: '', imagenPrincipal: '', imagenesAdicionales: [], imagenes: [], caracteristicas, zonaSnapshot: snapshot.zona, coeficientesAplicados: result.coeficientesAplicados, valorBase: result.valorBase, valorTerreno: result.valorTerreno, ...(snapshot.tipoPropiedad === 'terreno' ? {} : { valorConstruccion: result.valorConstruccion }), valorFinal: result.valorFinalEstimado, valorM2: result.valorM2, rangoMercado: result.rangoMercado, nivelConfianza: result.nivelConfianza, pricePerManzana: result.pricePerManzana, adjustedPriceM2: result.adjustedPriceM2, indiceLiquidez: result.indiceLiquidez, tiempoEstimadoVenta: result.tiempoEstimadoVenta, precioRecomendadoPublicacion: result.precioRecomendadoPublicacion, precioMinimoNegociacion: result.precioMinimoNegociacion, precioObjetivoCierre: result.precioObjetivoCierre, factorGlobal: result.factorGlobal };
