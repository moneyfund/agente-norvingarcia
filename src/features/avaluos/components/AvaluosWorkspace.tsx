  ...
  return <main className='min-h-screen bg-gradient-to-br from-[#0b0f17] via-[#111827] to-[#10233c] px-4 py-10'>
    <div className='mx-auto max-w-7xl'>
      ...
      <div className='mt-8 rounded-2xl border border-amber-500/30 bg-slate-950/60 p-6'>
        ...
        <div className='mt-6'>{propertyType==='terreno'&&<TerrenoForm value={form} onChange={(k,v)=>setForm((p)=>({ ...p,[k]:v }))} onSubmit={()=>submit(form,propertyType)} loading={loading} />}{propertyType==='casa'&&<CasaForm value={form} onChange={(k,v)=>setForm((p)=>({ ...p,[k]:v }))} onSubmit={()=>submit(form,propertyType)} loading={loading} />}</div>
      </div>
      <AvaluoTerrenoResultCard result={result} canSave={!!user} onSave={save} />
