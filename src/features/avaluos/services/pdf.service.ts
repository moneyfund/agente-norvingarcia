...
const normalizeCoeficientes = (coeficientesAplicados: any) => Array.isArray(coeficientesAplicados)
  ...
  : Object.entries(coeficientesAplicados || {}).map(([factor, coeficiente]) => ({ factor, valorAplicado: Number(coeficiente).toFixed(3), coeficiente, impacto: impactText(coeficiente) }));
...
export const exportAvaluoToPdf = (avaluo: any) => {
  ...
  const terreno = avaluo?.tipoPropiedad === 'terreno' ? `
    ...
    <table>
      ...
      <tr><td>Servicios</td><td>${list(c.servicios)}</td></tr>
  ...
  const html = `<!doctype html><html><head><style>body{font-family:Arial,sans-serif;color:#1e293b;padding:32px}h1{color:#0f172a}h2{margin-top:24px;color:#334155}table{width:100%;border-collapse:collapse;margin-top:12px}td,th{border:1px solid #cbd5e1;padding:8px;text-align:left}th{background:#f1f5f9}.highlight{background:#ecfdf5;border:1px solid #a7f3d0;padding:14px;border-radius:10px;margin:12px 0}</style></head><body><h1>Informe Técnico Inmobiliario</h1><p>Fecha: ${new Date().toLocaleDateString('es-NI')}</p><h2>${avaluo.titulo}</h2><p>Zona: ${avaluo.zona}, ${avaluo.ciudad}</p>${terreno}<div class='highlight'><strong>Valor final:</strong> ${money(avaluo.valorFinal)}</div><h2>Coeficientes aplicados</h2><table><thead><tr><th>Factor</th><th>Valor aplicado</th><th>Impacto</th></tr></thead><tbody>${rows}</tbody></table><p>Firma profesional: Norvin García Real Estate</p></body></html>`;
